...
Nightmare.prototype.goto = function(url, headers) {
  debug('queueing action "goto" for %s on queue %d', url,this._queueID);
  var self = this;
...
Nightmare.prototype.run = function(fn) {
  debug(`running queue ${this._queueID}`)
  var steps = this.enqueue();
  this.running = true;
  this._queues[this._queueID] = [];
  var self = this;
  ...
  function next (err, res) {
    if (this._inSync >= 0 && this._queueID != this._inSync) {
      debug('suspending queue %d',this._queueID);
      this._queues[this._queueID] = steps.concat(this._queues[this._queueID]);
      this._runQueue.push(fn);
      return;
    } else if (this._inSync >=0) {
      debug('running synced queue %d',this._queueID)
    }
  ...
  function done () {
    ...
    self.running = false;
    if (self.ending) {
      if (self._inSync < 0 && self._finalizer == null) {
        debug('running end instance');
        return endInstance(self, () => fn.apply(self, doneargs));
      } else if (self._finalizer == null) {
        debug('sidequeing end instance');
        self._finalizer = fn;
        return self;
      }
    }
...
Nightmare.prototype.sync = function(fn) {
  debug('suspending queues %d', this._inSync);
  var newQueue = this.queue();
  if (this._inSync == -1) {
    this._inSync = newQueue._queueID;
    fn(newQueue);
  } else {
...
Nightmare.prototype.unsync = function() {
  debug('unsync called %d', this._inSync);
  this._inSync = -1;
  if (this._syncQueue.length > 0) {
    this.sync(this._syncQueue.shift());
    return;
  }
  debug('resuming run queues: %d',this._runQueue.length);
...
Nightmare.prototype.evaluate_now = function(js_fn, done) {
  ...
  var id = (this._scriptCounter++);
  var source = template.execute({ src: String(js_fn), args: argsList, id: id });

  this.child.call('javascript', source, id, done);
  return this;
};
...
Nightmare.prototype._inject = function(js, done) {
  var id = (this._scriptCounter++);
  this.child.call('javascript', template.inject({ src: js, id: id }), id, done);
  return this;
};
...
Nightmare.prototype.enqueue = function(done) {
  debug('enqueue item on queue %d',this._queueID);
  if (!arguments.length) return this._queues[this._queueID];
  var args = sliced(arguments);
  var fn = args.pop();
  this._queues[this._queueID].push([fn, args]);
};
...
Nightmare.prototype.queue = function() {
  if (this.child == undefined) {
    throw "Please get new queues within a run";
  }
  nextID = ++this._queueCounter;
  var handler = {
    id: nextID,
    get: function(target,name,receiver) {
      if (name == '_queueID') {
        debug('referencing queue %d',this.id);
        return this.id;
      }
      if (name == '__proxy') {
        return true;
  ...
  debug('returning new queue with id %d',handler.id);
  this._queues[handler.id] = [];
...
function queued (name, fn) {
  return function action () {
    debug('queueing action "' + name + '" on queue %d',this._queueID);
